...
<html>
	...
	function checkusername(field, url) {
		...
		if (uername == "" || uername.length < 3) {
			...
			if (null != xmlHttpRequest) {
				...
				xmlHttpRequest.open("get", url + "?admin.username=" + uername,
						true);
				//关联好ajax的回调函数
	...
	<center>
		<body>
			<center>
				<font class="tabletop">添加进货商</font>
			</center>
			<br />
			<br />
			<!--<c:set var="stu" value="${sessionScope.stu_info }"/>-->
			<form action="InputerAction!addInputer" target="mainFrame">
				<table class="ta" width="200px">
					<tr>
						...
						<td>
							<input type="text" name="inputer.name" value="${session.inputer.name }"
