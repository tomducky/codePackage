...
<html>
	...
	$(function() {
		$(".ta tr").each(function(i) {
          ...
          $("#intoDate").datepicker({ dateFormat: 'yy-m-d' });
	...
	<center>
	<body>
		<center><font class="tabletop">修改进货</font></center><br/><br/>
		<!--<c:set var="stu" value="${sessionScope.stu_info }"/>-->
		<form action="IntoGoodsAction!updateIntoGoods" target="mainFrame" onsubmit="javascript:window.close();">
		<table class="ta" width="200px" >	
		<tr >
			<td>进货时间</td>
			<td><input type="text" name="intoGoods.intoDate" id="intoDate" value="${request.intoGoods.intoDate }"/>
				<input type="hidden" name="intoGoods.id" id="intoDate" value="${request.intoGoods.id }"/>
			</td>
		</tr>
		<tr >
			<td>进货数量</td>
			<td><input type="text" name="intoGoods.num"  value="${request.intoGoods.num }"/>
			</td>
		</tr>
		
		<tr >
			<td>
				<s:select list="%{#request.map}" name="intoGoods.goods.id" headerKey="-1" headerValue="请选择"  label="进货商品"></s:select>
            </td>
		</tr>
		<tr>
